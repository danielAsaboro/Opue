...
import { LayoutGrid, Table, Star, TrendingUp, Download, Map } from 'lucide-react';
...
export default function PNodesPage() {
    ...
    const displayPNodes = showFavoritesOnly
        ? filteredPNodes.filter((p) => favorites.includes(p.id))
        : filteredPNodes;
    ...
    const exportData = () => {
        const csv = [
            ['ID', 'Status', 'Storage', 'Performance', 'Uptime', 'Location'].join(','),
            ...displayPNodes.map((p) =>
                [
                    p.id,
                    p.status,
                    formatBytes(p.storage.capacityBytes),
                    p.performanceScore,
                    formatPercentage(p.performance.uptime),
                    p.location || 'Unknown',
                ].join(',')
            ),
        ].join('\n');
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Browse and analyze all storage providers
                    </p>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                    <Button variant="outline" onClick={exportData} disabled={displayPNodes.length === 0}>
            ...
            {/* Filters */}
            {allPNodes && (
                <PNodeFilters pnodes={allPNodes} onFilterChange={setFilteredPNodes} />
            )}
            ...
            {/* Content */}
            {viewMode === 'table' && (
                <PNodeTable pnodes={displayPNodes} isLoading={false} />
            )}
